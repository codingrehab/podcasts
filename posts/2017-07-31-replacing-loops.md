...
Here we see an example of incrementing the elements of a collection one at a time:
...
> Why use other functions instead of just list comprehensions or even a custom _loop_? 
...
On the other hand, when using well known functions we are applying clearly defined building blocks that we do not need to explain nor provide examples. All that effort comes as a boon and make our code more idiomatic as well.

## Combining blocks

Using common building blocks make code expressive and very easy to understand.

The more familiar we are with these blocks the easier is for us to combine them to satisfy our needs.

Here is an example of finding the first event element in a collection: 
...
Here is another example taking all the numbers that are greater than zero, incrementing them by one and filtering the even ones. 
...
The `map` function will expect a function that has the arity matching the collections passed as parameter. For two collections a function that received two parameters, for three collections a function that received three parameters and son on.
...
Reducing a collection is used to transform no only one element at a time but to have an accumulator that can help us decide what to do.

The `reduce` function takes first a function with two parameters (the accumulator and the current element), then an initial value and last the collection to reduce.
